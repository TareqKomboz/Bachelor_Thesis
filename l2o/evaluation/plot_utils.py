...
def plot(step_counter, plot_dir, function_values):
    ...
    average_reward_over_batches_and_actions = tf.reduce_mean(function_values).numpy()
    reward_stds_over_batches_and_actions = tf.math.reduce_std(function_values).numpy()

    average_return_over_batch = tf.reduce_mean(tf.reduce_sum(input_tensor=function_values, axis=1, keepdims=True)).numpy()
    ...
    average_final_objective_function_value_over_batch = tf.reduce_mean(function_values[:, -1]).numpy()
    final_objective_function_value_stds_over_batch = tf.math.reduce_std(function_values[:, -1]).numpy()

    average_max_reward_of_episode_over_batches = tf.reduce_mean(tf.reduce_max(function_values, axis=1)).numpy()
    max_reward_of_episode_stds_over_batches = tf.math.reduce_std(tf.reduce_max(function_values, axis=1)).numpy()
